...
export function HeroStorySection() {
  ...
  const startScene = useCallback((idx: number) => {
    ...
    timersRef.current.push(
      setTimeout(() => {
        ...
        if (headlineRef.current)
          headlineRef.current.innerHTML = `${sd.headline[0]}<br/><span style="color:${idx === 1 ? '#00A8FF' : '#fff'}">${sd.headline[1]}</span>`
    ...
    // Next scene
    timersRef.current.push(
      setTimeout(() => startScene((idx + 1) % SCENES.length), sd.duration)
    )
    ...
    const tick = (now: number) => {
      ...
      if (progressRef.current)
        progressRef.current.style.width = `${rawF * 100}%`
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(to right,rgba(8,12,24,0.72) 0%,rgba(8,12,24,0.28) 60%,rgba(8,12,24,0.05) 100%)',
        }}
      />
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(to top,rgba(8,12,24,0.95) 0%,transparent 55%)',
        }}
      />
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(to bottom,rgba(8,12,24,0.45) 0%,transparent 18%)',
        }}
      />
