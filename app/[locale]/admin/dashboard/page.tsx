        ...
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <div className="flex items-center justify-between rounded-lg bg-white/10 p-4 backdrop-blur-sm">
            ...
            <Button
              ...
              variant="outline"
              className="flex items-center gap-2 border-white/30 px-4 py-1.5 text-sm text-white hover:bg-white/10 hover:text-white"
            >
          ...
          {uploadStatus === 'success' && uploadedFile ? (
            ...
            <Card className="overflow-hidden border-white/20 bg-white/10 backdrop-blur-sm">
              <CardHeader className="relative bg-gradient-to-r from-white/10 to-white/5">
                <Button
                  variant="ghost"
                  size="sm"
...
/**
 * AdminDashboard — protected page for uploading team data.
 * Only accessible to authenticated team managers.
 */
export default function AdminDashboard() {
  const { locale } = useParams<{ locale: string }>()

  return (
    <ProtectedRoute redirectTo={`/${locale}/admin`}>
      <DashboardContent />
    </ProtectedRoute>
  )
}
